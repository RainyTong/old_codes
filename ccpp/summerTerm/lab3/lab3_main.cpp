...
/*function to get hash_code for one file
* FileNameInPut:文件路径
* outSHA1:SHA1输出变量
* outError:错误信息输出变量
* returns:outSHA1
...
char *GetFileSHA1(char *FileNameInPut, char *outSHA1, char *outError)
{
    ...
    if (!fp)
    {
        if (outError != NULL)
        {
            sprintf(outError, "fail to open \"%s\"", FileNameInPut);
        }
        return outSHA1;
    }
    sha1_init (&ctx);
    ...
    if (ferror (fp))
    {
        if (outError != NULL)
        {
            sprintf(outError, "fail to read \"%s\"", FileNameInPut);
        }
        return outSHA1;
    }
    sha1_final (&ctx);
...
void get_Hashcode ()
{
	 ...
	 for(i = 0; i<sizeof(data) ; i++){
		 ...
		 if (strlen(eror) != 0)
		 {
			 ...
			 data[i].hash_code = GetFileSHA1(data[i].namel, sha1, eror) ;
...
int main(){
	printf("Please enter the file you want to scan :\n");
	char path[20] ;
