...
//the secure hashcode arithmetic
...
//the function to get the hash_code for all file in the struct
void get_Hashcode(int size)
{

	 int i=0;
	 	while(data[i].namel[0]!='\0'){
	   ...
	   char eror[256] = { 0 };
		 if (strlen(eror) != 0)
		 {
			 ...
			 data[i].hash_code = GetFileSHA1(data[i].namel, data[i].hash, eror) ;

			 // print some "<" to illustrustrate the progress
			 if(i<=size){
				 int t = 0;
				 int number = 30/size ;
				 if(number!=0){
